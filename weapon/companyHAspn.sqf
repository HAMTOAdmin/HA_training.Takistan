...
switch _this do {
	...
	case "KO" : {
		_items = _items+[_Bandanna,_Scarf,_Rangefinder,_G];
		_weapSecd = [_RPGO];
	...
	//Разведчик снайпер
	case "SN" : {
		_items = _items+[_Bandanna,_Scarf,_Rangefinder];
	...
	case "M" : {
		_items = _items+[_Bandanna,_Scarf];
		_weapPrim = [_Rifle,[_Collim,_DTK]];
		_weapHand = [_Pistol,[_PistolSilencer,_PistolIR]];
		_uniforms set [1, (_uniforms select 1)+[
				[_BERUSHI, 1],
	...
	//Старший разведчик
	case "SS" : {
		_items = _items+[_Bandanna,_Scarf,_Binocular,_G];
		_weapSecd = [_RPGO,[]];
